...
//  StackPinLayout
//
//  Created by Muhammad Ghifari on 10/01/22.
//
...
class BottomSheetControllerView: UIView{
    ...
    override func layoutSubviews() {
        if !isDragging{
            ...
            if isFullScreen{
                container.pin.top(pin.safeArea.top + 24).left().right().bottom()
            } else{
                container.pin.left().right().bottom().wrapContent(.vertically, padding: UIEdgeInsets(top: 12, left: 0, bottom: 8, right: 0))
            }
